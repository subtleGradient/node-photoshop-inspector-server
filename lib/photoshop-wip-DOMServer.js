...
DOMServer.prototype.setAttributeValue = function setAttributeValue(params, callback, fire){
...
DOMServer.prototype['ao_set:data'] = function(params, callback, fire){
  ...
  if (requestedPath.protocol == null || requestedPath.protocol == 'file:'){
    fs.exists(requestedPath.path, function(exists){
      if (exists) return setPath(requestedPath.path, callback);
      ...
      photoshop.invoke('app.activeDocument.fullName.fsName.toString', function(error, activeDocument_path){
        ...
        fs.exists(relativePath, function(exists){
          if (exists) return setPath(relativePath, callback);
  ...
  function setPath(path, callback){
    photoshop.invoke(
      function setData(layerID, path){
        PSFakeDOM.debug = true;
        PSFakeDOM.selectLayerByRef(PSFakeDOM.getLayerRefById(layerID));
        ao_executeAction.placedLayerReplaceContents(path);
        // PSFakeDOM.executeActionGet(PSFakeDOM.getLayerRefById(layerID))
      },
      [params.nodeId, path],
      function(error, result){
        if (error) return callback(error);
        fire({method:"DOM.attributeModified", params:{nodeId:params.nodeId, name:'data', value:params.value}})
        callback(null)
      }
    )
  }
...
DOMServer.prototype.setAttributesAsText = function setAttributesAsText(params, callback, fire){
  var DOM = this
