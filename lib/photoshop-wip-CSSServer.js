...
function CSSStyle(object, id, prefix){
  if (!prefix) prefix = ''
  if (id) this.styleId = {styleSheetId:JSON.stringify(id), ordinal:id.nodeId || 0}
...
CSS.getInlineStylesForNode = function(params, callback, fire){
  ...
  css._context.photoshop.invoke(
    function(layerID){
      var result = {}
      result.layer = PSFakeDOM.executeActionGet(PSFakeDOM.getLayerRefById(layerID))
      try {
        result.metadata = PSFakeDOM.getMetaDataForLayerId(layerID)
        result.parsedMetadata = ao_XMP.parse(result.metadata.ao_getValue(stringIDToTypeID('layerXMP')))
      } catch(e){}
      return result
    },
    params.nodeId,
    function(error, result){
      ...
      if (!result.layer) return callback(Error('layer not returned'));
      
      var layer = result.layer
      
      if (!(layer && layer.bounds))  return callback(Error('layer with layerID:"' + params.nodeId + '" not found'));
      
      layer.parsedMetadata = result.parsedMetadata
      layer.metadata = result.metadata
...
CSS._inlineStyleForLayer = function(layer){
  ...
  editableProperties['-aops-innerHTML'] = this._styledTextContentForLayer(layer)
  if (!editableProperties['-aops-innerHTML']) delete editableProperties['-aops-innerHTML'];
  
  if (layer.metadata){
    editableProperties.updated = Date(layer.metadata.layerTime * 1000)
  }
  
  if (layer.smartObject){
    editableProperties.source = 'url(' + (layer.parsedMetadata && layer.parsedMetadata.source || './' + layer.smartObject.fileReference) + ')'
  }
  
  return new CSSStyle(editableProperties, {nodeId:layer.layerID, key:'editableProperties', getterKey:'_inlineStyleForLayer'})
}

CSS._attributesStyleForLayer = function(layer){
  return new CSSStyle(Object_flatten(layer), {nodeId:layer.layerID, getterKey:'_attributesStyleForLayer'}, '-ps-')
}

CSS._styledTextContentForLayer = function(layer){
  if (!(layer.textKey && layer.textKey.textStyleRange)) return;
  var spans = [], markup = layer.textKey.textKey.split('')
  
  layer.textKey.textStyleRange.forEach(CSS._styledTextContentForLayer_collectRange, spans)
  layer.textKey.paragraphStyleRange.forEach(CSS._styledTextContentForLayer_collectRange, spans)
  
  for (var span, attrs, index = spans.length; --index >= 0;){
    span = spans[index]
    if (span == null) continue;
    span.sort(function(a,b){ return ((a.closeTag?1<<1:0<<1) | (a.paragraphStyle?1<<2:0<<2)) - ((b.closeTag?1<<1:0<<1) | (b.paragraphStyle?1<<2:0<<2))})
    
    for (var spanItem, spanIndex = -1, spanLength = span.length; ++spanIndex < spanLength;) {
      spanItem = span[spanIndex]
      attrs = ''
      // if (spanItem.openTag){
      //   attrs = ' ' + Object.keys(spanItem.paragraphStyle || spanItem.textStyle).map(function(key){ return key + '=' + JSON.stringify(this[key]) }, spanItem.paragraphStyle || spanItem.textStyle).join(' ')
      // }
      markup.splice(index, 0, (spanItem.closeTag?'</':'<') + (spanItem.paragraphStyle ? 'p' : spanItem.textStyle.fontStyleName.charAt(0)) + attrs + '>')
    }
  }
...
// CSS._markupForTextStyle = function(textStyle, close){
//   var tag = textStyle
//   return (close ? '</' : '<') + tag
// }

CSS._styledTextContentForLayer_collectRange = function(range, index){
  if (this[range.from] == null) this[range.from] = []
  if (this[range.to] == null) this[range.to] = []
  this[range.from].unshift({openTag:1, textStyle:range.textStyle, paragraphStyle:range.paragraphStyle})
  this[range.to].push({closeTag:1, textStyle:range.textStyle, paragraphStyle:range.paragraphStyle})
}
...
CSS.getMatchedStylesForNode = function(params, callback, fire){
  ...
  function doTheThing(error, layer){
    ...
    // theThings.matchedCSSRules.unshift(createRuleMatch(layer, '.metadata', result.metadata, 'meta'))
...
function downloadImage(_url, callback){
  ...
  if (_url.protocol == null || _url.protocol == 'file:'){
    path = _url.pathname
    // FIXME: Add support for relative paths
    return callback(null, path)
  }
  
  if (_url.protocol == 'http:' || _url.protocol == 'https:'){
    path = process.env.TMPDIR + '/' + encodeURIComponent(_url.href)
    var stream = request(_url.href).pipe(fs.createWriteStream(path))
    stream.on('end', function(){ callback(null, path) })
    stream.on('error', callback)
    return
  }
  
  callback(Error('Protocol "' + _url.protocol + '" is not supported'))
}


CSS.setPropertyText = function(params, callback, fire){
  var self = this;
  var parsedStyleId = JSON.parse(params.styleId.styleSheetId)
  var nodeId = parsedStyleId.nodeId
  
  var parsed = params.text.match(/^([^:]*):\s*(.*?);$/)
  var key = parsed[1]
  var value = parsed[2]
  ...
  if (key == 'source'){
    downloadImage(value.replace(/^url\(["']?|['"]?\)$/ig,''), function(error, path){
      if (error) return callback(error)
      console.log(path)
      doIt(key, path)
    })
  ...
  function doIt(key, value){
    // TODO: handle params.overwrite and params.propertyIndex
    self._context.photoshop.invoke(
      function(layerID, key, value){
        var layerRef = PSFakeDOM.layerRefForId(layerID)
        var values = {}
        values[key] = value
        PSFakeDOM.setLayer(layerRef, values)
        return app.executeActionGet(layerRef)
      },
      [nodeId, key, value],
      function(error, layer){
        if (error) return callback(error);
        fire({method:'CSS.styleSheetChanged', params:{styleSheetId:params.styleId.styleSheetId}})
        callback(null, {style:CSS[parsedStyleId.getterKey](layer)})
      }
    )
  }
}
